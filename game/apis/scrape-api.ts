...
async function fetchListings(city: string, type: string, from: number, size: number) {
...
export async function scrapeApi(searchParams: SearchParams): Promise<ScrapedListingsModel> {
    // Step 1: Get total count
    const initial = await fetchListings(searchParams.city, searchParams.estateType, 0, 1);
    ...
    const midData = await fetchListings(searchParams.city, searchParams.estateType, mid - 10, 20);
    ...
    while (left < right) {
        ...
        const check = await fetchListings(searchParams.city, searchParams.estateType, midPoint, 1);
    ...
    const final = await fetchListings(searchParams.city, searchParams.estateType, Math.max(0, left - 5), 50);
